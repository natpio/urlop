...
class PRO_TransportOrder(FPDF):
    ...
    def footer(self):
        ...
        # --- Zaktualizowana logika przypisywania kontaktów ---
        if self.opiekun == "PD":
            email = "piotr.dukiel@sqm.eu"
            phone = "+48 577 636 367"
        elif self.opiekun == "AW":
            email = "alicja.wojciechowska@sqm.eu"
            phone = "+48 570 330 290"
        else:
            email = "logistics@sqm.eu"
            phone = ""
            
        contact_str = f"Generated by Global Logistics Terminal | {email} | {phone}"
        self.cell(0, 5, pdf_sanitize(contact_str), ln=True, align='C')
...
<p style="color:#CBD5E1;">Zautomatyzowane generowanie zasileń PDF i rejestracja w bazie Zlecenia.</p>
    ...
    if "pdf_data" in st.session_state:
        ...
        with st.container(border=True):
            ...
            # --- Zaktualizowana nazwa w opcjach wyboru ---
            podpis = u2.radio("Podpis:", ["PD", "AW"], index=(["PD", "AW"].index(val_podpis) if val_podpis in ["PD", "AW"] else 0), horizontal=True)
