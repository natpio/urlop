...
class PRO_TransportOrder(FPDF):
    ...
    def footer(self):
        ...
        email = "piotr.dukiel@sqm.eu" if self.opiekun == "PD" else "piotr.dukiel@sqm.eu"
...
if nav_mode == "🌍 Hub Operacyjny":
    if st.session_state["role"] == "admin":
        ...
        with tab_a1:
            ...
            if st.button("🛫 Wgraj aktualizację zadań", type="primary"): conn.update(worksheet="Arkusz1", data=clean_for_gsheets(edytowane_zadania)); st.cache_data.clear(); st.rerun()
        with tab_a2:
            ...
            if st.button("🛫 Wgraj aktualizację harmonogramu", type="primary"): conn.update(worksheet="Harmonogram", data=clean_for_gsheets(edytowane_harm)); st.cache_data.clear(); st.rerun()
        with tab_a3:
            edytowane_przewoz = st.data_editor(df_carriers, num_rows="dynamic", use_container_width=True, hide_index=True)
            if st.button("🛫 Wgraj aktualizację floty", type="primary"): conn.update(worksheet="Przewoznicy", data=clean_for_gsheets(edytowane_przewoz)); st.cache_data.clear(); st.rerun()
        with tab_a4:
            ...
            if st.button("🛫 Wgraj aktualizację systemów", type="primary"): conn.update(worksheet="Linki", data=clean_for_gsheets(edytowane_linki)); st.cache_data.clear(); st.rerun()
        with tab_a5:
                ...
                with st.form("logbook_form_admin", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Wyślij do systemu ➔", use_container_width=True):
                        if kto.strip() != "" and wiadomosc.strip() != "":
                            ...
                            conn.update(worksheet="Notatnik", data=clean_for_gsheets(pd.concat([df_notes, nowy_wpis], ignore_index=True))); st.cache_data.clear(); st.rerun()
            with col_hist:
                ...
                for idx, row in df_notes_clean.iloc[::-1].iterrows():
                    ...
                    if st.button("✂️ CUT", key=f"del_{idx}"):
                        conn.update(worksheet="Notatnik", data=clean_for_gsheets(df_notes.drop(idx))); st.cache_data.clear(); st.rerun()
    ...
    if st.button(btn_label, type="primary", use_container_width=True):
        if not nazwa_przewoznika or nazwa_przewoznika == "Wybierz...":
            ...
            with st.spinner("Przetwarzanie dokumentów i bazy..."):
                ...
                st.download_button("📥 POBIERZ ZLECENIE PDF", data=pdf_bytes, file_name=f"Order_{nr_zlecenia.replace('/', '_')}.pdf", mime="application/pdf", use_container_width=True)
                st.cache_data.clear()
